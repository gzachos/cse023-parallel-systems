/* matmul_hybrid.c
 * ---------------
 * Parallel program using the hybrid programming model for matrix-matrix
 * product: MPI for parallelism between nodes and OpenMP for parallelism
 * within the SMP nodes.
 *
...
#define OMP_NTHR	4    /* 4 processors per socket */
...
int main(int argc, char **argv)
{
	...
	#pragma omp parallel for private(j,k,sum) schedule(guided) \
	                                          num_threads(OMP_NTHR)
		for (i = 0; i < WORK; i++)
			for (j = 0; j < N; j++)
			{
				for (k = sum = 0; k < N; k++)
